/*
 * We can also take a look at the performance of native C++ (enabled by Boost) 
 * against Rcpp Sugar (essentially R functions made available to C++). We can 
 * see from the Gibbs sampler example below (Taken form Hadley Wickham's 
 * Advanced R chapter) that Native C++ is roughly twice as fast. You can 
 * access the source file here
